...
<section x-data="carousel({{ json_encode($slides) }})" x-init="init()" class="relative w-full overflow-hidden bg-white py-12 rounded-xl">
    ...
    <div x-ref="trackContainer" 
         class="relative z-10 flex items-center justify-center overflow-visible px-6"
        ...
        <div class="flex" :class="{ 'transition-transform duration-1000': animate }"
             :style="trackStyle"
            ...
            <template x-for="(slide,index) in displaySlides" :key="index">
                <div class="flex-none" :style="`width: ${slideWidth}%; margin: 0 ${gap/2}%`">
                    ...
                    <div class="bg-cover bg-center aspect-video rounded-2xl overflow-hidden"
                         :style="`background-image: url('${slide.image}')`">
                        <div class="h-full flex flex-col items-center justify-end text-center text-light px-6 pb-20 bg-dark/40">
    ...
    <div class="absolute bottom-3 left-0 w-full flex justify-center space-x-4">
