    ...
    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow rounded p-6 mb-6">
            ...
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Price</p>
                    <p class="text-sm text-grey-dark mt-1">{{ number_format($product->price, 2) }}</p>
                </div>
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Promo Price</p>
                    <p class="text-sm text-grey-dark mt-1">{{ $product->promo_price ? number_format($product->promo_price, 2) : '—' }}</p>
                </div>
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Stock</p>
                    <p class="text-sm text-grey-dark mt-1">{{ $product->stock }}</p>
                </div>
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Production Time (days)</p>
                    <p class="text-sm text-grey-dark mt-1">{{ $product->production_time }}</p>
                </div>
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Weight (g)</p>
                    <p class="text-sm text-grey-dark mt-1">{{ $product->weight ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Dimensions (W × L × H mm)</p>
                    <p class="text-sm text-grey-dark mt-1">{{ $product->width ?? '—' }} × {{ $product->length ?? '—' }} × {{ $product->height ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Tax</p>
                    <p class="text-sm text-grey-dark mt-1">{{ optional($product->tax)->name ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Categories</p>
                ...
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Featured</p>
                ...
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Promo</p>
                    <p class="text-sm text-grey-dark mt-1">
                        @if($product->is_promo)
                            <span class="text-status-success font-bold">&#10003;</span>
                        @else
