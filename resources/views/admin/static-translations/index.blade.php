    ...
    <div class="py-6 max-w-6xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-light p-4 rounded shadow">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('admin.static-translations.create') }}" class="bg-accent-primary text-light px-4 py-2 rounded">Create</a>
            </div>
                ...
                <tbody>
                    @foreach ($items as $item)
                        <tr class="border-t">
                            ...
                            <td class="p-2">
                                <a href="{{ route('admin.static-translations.edit', $item) }}" class="text-sm text-accent-secondary">Edit</a>
                                <form method="POST" action="{{ route('admin.static-translations.destroy', $item) }}" class="inline-block ms-2">
                                    @csrf
                                    @method('DELETE')
                                    <button class="text-sm text-grey-dark" onclick="return confirm('Delete?')">Delete</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
