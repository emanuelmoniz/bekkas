        ...
        <form method="GET" class="mb-6 bg-white p-4 rounded shadow">
            <div class="grid grid-cols-1 lg:grid-cols-6 gap-4">
                ...
                <div class="flex justify-end gap-2">
                    <x-default-button type="button" onclick="window.location.href='{{ route('admin.orders.payloads.index') }}'">
                        Reset
                    </x-default-button>
            ...
            <table class="min-w-full border">
                ...
                <tbody>
                    @foreach ($payloads as $p)
                        <tr class="border-t">
                            <td class="px-3 py-2">
                                <a href="{{ route('admin.orders.payloads.show', $p) }}" class="font-medium text-accent-primary hover:text-accent-primary/90 no-underline">{{ optional($p->order)->order_number ?? ('#' . $p->order_id) }}</a>
                            </td>
                            ...
                            <td class="px-3 py-2 text-right">
                                <a href="{{ route('admin.orders.payloads.show', $p) }}" class="text-accent-primary hover:text-accent-primary/90 no-underline">View</a>
                            </td>
