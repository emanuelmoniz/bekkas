    ...
    <div class="py-6 max-w-4xl mx-auto sm:px-6 lg:px-8">
        <div class="mb-4">
            <nav class="flex gap-2 text-sm" aria-label="Admin orders subnav">
                <a href="{{ route('admin.orders.index') }}" class="px-3 py-2 rounded hover:bg-light">Orders</a>
                ...
                <a href="{{ route('admin.orders.payments.index') }}" class="px-3 py-2 rounded {{ request()->is('admin/orders/payments*') ? 'bg-grey-light' : 'hover:bg-light' }}">Payments</a>            </nav>
            </nav>
        </div>
            ...
            <div class="flex justify-between items-start gap-4">
                ...
                <div class="text-right flex flex-wrap justify-end items-center gap-2">
                    @if($payload->order)
                        ...
                        <a href="{{ route('admin.orders.checkouts.index', ['order_number' => $payload->order->order_number]) }}" class="inline-block bg-light border px-4 py-2 rounded text-sm ms-2">Checkouts</a>

                        <a href="{{ route('admin.orders.payments.index', ['order_number' => $payload->order->order_number]) }}" class="inline-block bg-light border px-4 py-2 rounded text-sm ms-2">Payments</a>
                    ...
                    <a href="{{ route('admin.orders.payloads.index') }}" class="inline-block bg-light border px-4 py-2 rounded text-sm ms-2">Back</a>
                </div>
            </div>

            <div>
                <h3 class="font-semibold mb-2">Stored payload (JSON)</h3>
                <pre class="whitespace-pre-wrap bg-light border rounded p-4 text-sm overflow-auto" style="max-height:48vh">{{ json_encode($payload->payload, JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES) }}</pre>
            </div>
        </div>
    </div>
</x-app-layout>
