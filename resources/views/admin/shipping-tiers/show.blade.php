    ...
    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow rounded p-6 mb-6">
            ...
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Weight From (g)</p>
                    <p class="text-sm text-grey-dark mt-1">{{ $tier->weight_from }}</p>
                </div>
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Weight To (g)</p>
                    <p class="text-sm text-grey-dark mt-1">{{ $tier->weight_to }}</p>
                </div>
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Cost (gross)</p>
                    <p class="text-sm text-grey-dark mt-1">{{ number_format($tier->cost_gross, 2) }}</p>
                </div>
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Shipping Days</p>
                ...
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Countries</p>
                    <p class="text-sm text-grey-dark mt-1">{{ $tier->countries->map(fn($c) => $c->name)->filter()->join(', ') ?: '—' }}</p>
                </div>
                <div>
                    <p class="text-xs text-grey-dark font-medium uppercase tracking-widest">Regions</p>
