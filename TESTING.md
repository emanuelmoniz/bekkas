...
- Make helper executable (once):
  - chmod +x bin/run-tests.sh
- Run full test suite (isolated sqlite DB) — when using the same server as production, clear the config cache first and then run the helper:
...
- (Optional) Create a local `.env.testing` file to override env values during tests:
  - cp .env.testing.example .env.testing
  - Edit `.env.testing` as needed (do NOT commit it; it's in `.gitignore`).
  - The test bootstrap will auto-load `.env.testing` when present so you don't need to export vars manually.

  Note: `.env.testing` is optional. If present, the test bootstrap will auto-load `.env.testing`. If `bootstrap/cache/config.php` exists and you want runtime overrides to take effect, run:
  - php artisan config:clear
Otherwise, proceed with `./bin/run-tests.sh`. Be aware that running migrations against a live database is destructive.
